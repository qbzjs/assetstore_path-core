...
namespace razz
{
    public class UpdateNote : EditorWindow
    {
        ...
        private static Vector2 _size = new Vector2(600, 540);
        ...
        private void OnGUI()
        {
            ...
            EditorGUILayout.LabelField("Interactor v0.94 brings so many features and improvements but also took so much time to develop.", _text);
            GUILayout.Space(10f);

            EditorGUILayout.LabelField("Next, v1.00 will replace all interaction types with a fully customizable structure and will add an AI to move the player for interaction.", _text);
            GUILayout.Space(2f);
#if UNITY_2019_3_OR_NEWER
            _linkStyle.normal.textColor = new Color(1f, 0.7f, 0.7f);
#else
            Rect example = new Rect(_padding, 100f, _size.x - 2f * _padding, 20f);
            if (example.Contains(Event.current.mousePosition))
                _linkStyle.normal.textColor = Color.white;
            else _linkStyle.normal.textColor = new Color(1f, 0.7f, 0.7f);
#endif
            if (GUILayout.Button("Here is a Short Video Demonstration of the Ideas", _linkStyle))
            {
                Application.OpenURL("https://www.youtube.com/watch?v=NhF33WVIQEo");
            }
            GUILayout.Space(20f);

            EditorGUILayout.LabelField("These goals are long road trip.", _text);
            GUILayout.Space(2f);
            EditorGUILayout.LabelField("You can't make the road shorter, but you can help me to go faster.", _text);
            GUILayout.Space(20f);
            EditorGUILayout.LabelField("Your reviews significantly increase the time I can focus on Interactor instead of doing other freelance projects to make a living.", _text);
            GUILayout.Space(2f);
            ...
            GUILayout.FlexibleSpace();
            GUILayout.BeginArea(new Rect(_padding * 2f, 398f, _size.x - _padding * 4f, 200f));
