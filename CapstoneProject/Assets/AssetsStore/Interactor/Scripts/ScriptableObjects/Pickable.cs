...
namespace razz
{
    ...
    public class Pickable : InteractionTypeSettings
    {
        ...
        [Range(0, 10f)] public float pickupSpeed = 4f;
        [Header("One Hand")]
        ...
        public override void Init(InteractorObject interactorObject)
        {
            ...
            _col = _intObj.col;
            _easer = Ease.FromType(_intObj.easeType);
        ...
        public override void UpdateSettings()
        {
            if (_intObj.interactionType == InteractionTypes.PickableOne)
            {
                ...
                if (_droppingToLocation)
                {
                    ...
                    _holdWeight = Mathf.Lerp(_holdWeight, 0, Time.deltaTime * _easer(_durationTarget));
                ...
                {//Pickready set from a single Pick() call, now it can pick object to desired hold location until drop event.
                    if (_holdWeight > 0.95f)
                    {
                        _pickReady = false;
                        return;
                    }
                    _holdWeight = Mathf.Lerp(_holdWeight, 1f, Time.deltaTime * _easer(_durationBack));
            ...
            else
            {
                ...
                if (twoHandPicked)
                {
                    if (_holdWeight > 0.98f) return;

                    _holdWeight = Mathf.Lerp(_holdWeight, 1, Time.deltaTime * _easer(_durationTarget, _intObj.speedCurve));
