...
namespace FIMSpace.FEyes
{
    ...
    public partial class FEyesAnimator : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            {
                if (EyeSetups[i].ControlType == EyeSetup.EEyeControlType.Blendhsape) continue;
        ...
        protected virtual void LateUpdate()
        {
            ...
            for (int i = 0; i < Eyes.Count; i++)
            {
                ...
                if (eyeSetup.ControlType == EyeSetup.EEyeControlType.Blendhsape)
                {
                    if (eyeSetup.BlendshapeMesh == null) continue;

                    Vector3 lookDir = Quaternion.Euler(-clampDelta) * Vector3.forward;
