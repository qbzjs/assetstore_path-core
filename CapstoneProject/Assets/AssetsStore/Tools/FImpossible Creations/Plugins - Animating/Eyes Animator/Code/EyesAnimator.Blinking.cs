...
namespace FIMSpace.FEyes
{
    public partial class FEyesAnimator
    {
        ...
        public List<Vector3> EyeLidsCloseScales;
            ...
            if (AdditionalEyelidsMotion > 0f)
            {
                if (upper)
                    if (UpEyelidsBlendShapes.Count > 0)
                    {
                        ...
                        if (deltaV.x > 1)
                        {
                            float openMoreFactor = Mathf.Lerp(0f, -0.475f , Mathf.InverseLerp(1, EyesClampVertical.y, deltaV.x));
                            ...
                            if (deltaV.x < -1)
                        {
                            float closeFactor = Mathf.Lerp(0f, 0.4f , Mathf.InverseLerp(-1, EyesClampVertical.x, deltaV.x));
                        ...
                        float value = Mathf.LerpUnclamped(lid.Open * AdditionalEyelidsMotion, lid.Closed * AdditionalEyelidsMotion, lid.AdditionalFactor);
                        value += lid.PreWeight;
                ...
                if (!upper)
                    if (DownEyelidsBlendShapes.Count > 0)
                    {
                        ...
                        if (deltaV.x > 1)
                        {
                            float closeFactor = Mathf.Lerp(0f, 0.3f , Mathf.InverseLerp(1, EyesClampVertical.y, deltaV.x));
                            ...
                            if (deltaV.x < -1)
                        {
                            float openMoreFactor = Mathf.Lerp(-1.9f , 0f, Mathf.InverseLerp(EyesClampVertical.x, -1, deltaV.x));
