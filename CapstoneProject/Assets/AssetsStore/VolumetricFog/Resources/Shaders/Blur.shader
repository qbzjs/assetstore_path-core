...
SubShader {
	...
	float4 fragBlurBeyondDepth (v2fCross i): SV_Target {
	...
	float4 fragBlur (v2fCross i): SV_Target {
		UNITY_SETUP_INSTANCE_ID(i);
		float depth01 = Linear01Depth(SAMPLE_RAW_DEPTH_TEXTURE_LOD(_CameraDepthTexture, float4(i.uv, 0, 0)).r);
		float2 depths;
		depths.x = Linear01Depth(SAMPLE_RAW_DEPTH_TEXTURE_LOD(_CameraDepthTexture, float4(i.uv3, 0, 0)).r);
		depths.y = Linear01Depth(SAMPLE_RAW_DEPTH_TEXTURE_LOD(_CameraDepthTexture, float4(i.uv4, 0, 0)).r);
		float2 depthDiffs = abs(depth01.xx - depths);
		float4 pixel0 = PIX(i.uv);
		if (any(depthDiffs > _BlurEdgeThreshold)) return pixel0;

		pixel0 = pixel0 * 0.2270270270 + (PIX(i.uv1) + PIX(i.uv2)) * 0.3162162162 + (PIX(i.uv3) + PIX(i.uv4)) * 0.0702702703;
		return pixel0;
	}	
	...
	Pass { // Pass 0: Blur horizontally with depth threshold
	    ZTest Always Cull Off ZWrite Off
       	Fog { Mode Off }
		CGPROGRAM
		#pragma vertex vertBlurH
		#pragma fragment fragBlurBeyondDepth
		#pragma fragmentoption ARB_precision_hint_fastest
		#pragma target 3.0
		ENDCG
	}
	
	Pass { // Pass 1: Blur vertically with depth threshold
	    ZTest Always Cull Off ZWrite Off
       	Fog { Mode Off }
		CGPROGRAM
