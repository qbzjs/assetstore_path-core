...
namespace razz
{
    ...
    {
        public const float Version = 0.94f;
        ...
        private void Start()
        {
            ...
            {
                Debug.LogWarning("Could not find main camera", this);
            }
        ...
        private void FixedUpdate()
        {
            ...
            for (int i = 0; i < intOjbComponents.Count; i++)
            {
                ...
                if (intOjbComponents[i].interactorObject == null || !intOjbComponents[i].interactorObject.gameObject.activeInHierarchy)
                {
                    for (int a = 0; a < effectorLinks.Count; a++)
                    {
                        if (effectorLinks[a].enabled && effectors[a].connected)
                            effectorLinks[a].ObjectExit(intOjbComponents[i].interactorObject);
                    }

                    if (intOjbComponents[i].interactorObject != null && intOjbComponents[i].interactorObject.gameObject == _activeDistanceIntObj)
                        _activeDistanceIntObj = null;

                    RemoveInteraction(intOjbComponents[i]);
                    continue;
                }
                ...
                if (intOjbComponents[i].interactorObject.interactionType == InteractionTypes.DistanceCrosshair)
                    continue;
            ...
            for (int i = 0; i < effectorLinks.Count; i++)
            {
                ...
                effectorLinks[i].Update(intOjbComponents, sphereColWithRotScale);
            }
        ...
        {
            intOjbComponents[i].distanceSqr = (_playerCenter - intOjbComponents[i].interactorObject.transform.position).sqrMagnitude;
        }
        ...
        private void IntObjEnter(GameObject Object)
        {
            ...
            if (selectedByUI < intOjbComponents.Count && intObj.interactionType != InteractionTypes.SelfItch && intOjbComponents[selectedByUI].interactorObject == intObj)
            {
        ...
        private void OnTriggerEnter(Collider collidedObj)
        {
            ...
            if (selectedByUI < intOjbComponents.Count && intOjbComponents[selectedByUI].interactorObject == intObj)
            {
                NewLookOrder(intObj, Look.OnSelection);
            }
            else
        ...
        private void RemoveInteraction(IntObjComponents removed)
        {
            ...
            intOjbComponents.Remove(removed);
        }
        #endregion

        #region Connect & Disconnect Effectors
        //Starts or stops interaction with sent InteractorObject
        public void StartStopInteraction(InteractorObject interactorObject, bool click)
        {
            for (int i = 0; i < intOjbComponents.Count; i++)
            {
                if (intOjbComponents[i].interactorObject == interactorObject)
                {
                    for (int j = 0; j < effectorLinks.Count; j++)
                    {
                        ...
                        effectorLinks[j].StartStopInteractionThis(intOjbComponents, i, click);
                    }
                }
            }
        }
            ...
            for (int i = 0; i < intOjbComponents.Count; i++)
            {
                if (intOjbComponents[i].interactorObject == interactorObject)
                {
                    for (int j = 0; j < effectorLinks.Count; j++)
                    {
                        ...
                        effectorLinks[j].StartStopInteractionThis(intOjbComponents, i, false);
                    }
                }
            }
        }
        ...
        {
            if (selectedByUI >= intOjbComponents.Count) return false;
            if (intOjbComponents[selectedByUI].interactorObject == interactorObject)
                    return true;
        ...
        public void AddInteractionManual(InteractorObject interactorObject)
        {
            if (intOjbComponents.Count > 0)
            {
                for (int i = 0; i < intOjbComponents.Count; i++)
                {
                    if (intOjbComponents[i].interactorObject == interactorObject) return;
                }
            }
            ...
            if (selectedByUI < intOjbComponents.Count && interactorObject.interactionType != InteractionTypes.SelfItch && intOjbComponents[selectedByUI].interactorObject == interactorObject)
            {
        ...
        public class EffectorLink
        {
            ...
            public void InterruptTransfer()
            {
                if (!_cachedTransfer)
                {
                    ...
                    _transferFromIntObj.RemoveEffectorFromUseables(_this);
                    _transferToIntObj.RemoveEffectorFromUseables(_this);
            ...
            {
                _interactorIK.StartInteraction(effector, interactorTarget, interactorObject);
            ...
            public void Update(List<IntObjComponents> interactionObjsInRange, Vector3 sphereColWithRotScale)
            {
                ...
                for (int objPlaceInList = 0; objPlaceInList < interactionObjsInRange.Count; objPlaceInList++)
                {
                                    ...
                                    if (_interactor.effectors[_this].connectedTo != _interactorObject)
                                    {
                                        ...
                                        if (passed)
                                        {
                                            ...
                                            {//If this effector in use for other interaction, try other possible effector if that one is not connected instead
                                                int otherUseable = _interactorObject.GetOtherUseableEffector(_this);
                                                if (otherUseable >= 0 && !_interactor.effectors[otherUseable].connected)
                                                    continue;
                                            }
                                        ...
                                        else _interactorObject.RemoveEffectorFromUseables(_this);
                                    }
                                ...
                                for (int i = 0; i < _allTargets.Length; i++)
                                {
                                    ...
                                    if (_interactor.effectors[_this].connectedTo != _interactorObject)
                                    {
                                        ...
                                        if (passed)
                                        {
                                            ...
                                            _interactorObject.AddEffectorToUseables(_this, 2);
                                            _selfPossible = false;
                                        }
                                        else _interactorObject.RemoveEffectorFromUseables(_this, 2);
                                    }
                                    else if (_interactor.effectors[_this].connected)
                                    {
                    ...
                    if (_interactorObject)
                    {
                        ...
                        _lastAddToUseables = _interactorObject.CanEffectorUse(_this);
            ...
            public void StartStopInteractionThis(List<IntObjComponents> intOjbComponents, int selectedByUI, bool click)
            {
                ...
                if (click)
                {
                    ...
                    if (startStopObject && startStopObject.interactionType == InteractionTypes.DistanceCrosshair)
                    {
                        ...
                        startStopObject.RemoveEffectorFromUseables(_this);
                ...
                if (selectedByUI < intOjbComponents.Count)
                {
                    ...
                    if (_interactor.effectors[_this].connected)
                    {
                        ...
                        if (startStopObject != _interactor.effectors[_this].connectedTo)
                        {
                            ...
                            if (!startStopObject.CanEffectorUse(_this)) return;
                            ...
                            if (!startStopObject.multipleConnections)
                            {
                                ...
                                {
                                    int otherUseable = startStopObject.GetOtherUseableEffector(_this);
                                    if (otherUseable >= 0 && !_interactor.effectors[otherUseable].connected)
                                    {
                                        startStopObject.RemoveEffectorFromUseables(_this);
                                        return;
                                        ...
                                        startStopObject.RemoveEffectorFromUseables(otherUseable);
                                }
                            }
                            ...
                            if (_interactor.effectors[_this].connectedTo.interruptible && _interactor.effectors[_this].connectedTo.priority <= startStopObject.priority)
                            {
                                if (_interactor.effectors[_this].connectedTo.interactionType != InteractionTypes.MultipleCockpit)
                                {
                                    ...
                                    if (startStopObject.multipleConnections)
                                    {
                                        ...
                                        if (otherUseable >= 0 && _interactor.effectors[otherUseable].connected)
                                        {
                                            _interactor.DisconnectAllForThis(_interactor.effectors[otherUseable].connectedTo);
                                        }
                                    }
                            ...
                            case InteractionTypes.ClimbableLadder:
                                {
                                    ...
                                    if (!_interactor.effectors[_this].connectedTo.used)
                                    {
                                        ...
                                        _interactor.effectors[_this].connectedTo.RemoveEffectorFromUseables(_this);
                            ...
                            case InteractionTypes.MultipleCockpit:
                                {
                                    ...
                                    startStopObject.multipleSettings.MultipleOut();
                                ...
                                case InteractionTypes.ManualButton:
                                    {
                                        ...
                                        if (!startStopObject.CanEffectorUse(_this)) continue;
                                ...
                                case InteractionTypes.MultipleCockpit:
                                    {
                                        ...
                                        startStopObject.multipleSettings.MultipleIn();
                                        startStopObject.RotatePivot(_effectorWorldSpace);
                                    ...
                                    case InteractionTypes.ManualButton:
                                        {
                                            ...
                                            if (!intOjbComponents[i].interactorObject.CanEffectorUse(_this)) continue;

                                            if (_interactor.vehiclePartsActive)
                                            {
                                                _interactor.vehiclePartCont.Animate(intOjbComponents[i].interactorObject.vehiclePartId, true, _interactor);
                                            }
                                            intOjbComponents[i].interactorObject.RemoveEffectorFromUseables(_this);
