...
namespace MHLab.Patch.Core.Client
{
    public interface ILauncherSettings : ISettings
    {
        ...
        bool EnableRepairToTheLatestVersionIfNoPatchFound { get; set; }
    }
    ...
    public class LauncherSettings : Settings, ILauncherSettings
    {
        ...
        public virtual FilePath GetSettingsOverridePath() =>
            new FilePath(RootPath, PathsManager.Combine(RootPath, "settings.json"));

        public virtual string GetDownloadedPatchArchivePath(IVersion from, IVersion to) =>
            PathsManager.Combine(GetTempPath(), string.Format(PatchFileName, from, to));

        public virtual string GetUncompressedPatchArchivePath(IVersion from, IVersion to) =>
            PathsManager.Combine(GetTempPath(), string.Format("{0}_{1}_uncompressed", from, to));
        ...
        public virtual string GetUpdaterSafeModeBackupPath() =>
            PathsManager.Combine(GetTempPath(), "UpdaterSafeModeBackup");

        public virtual FilePath GetUpdaterSafeModeLockFilePath() => new FilePath(GetTempPath(),
            PathsManager.Combine(GetTempPath(), "patch_tmp_safemode_lock"));

        public string RemoteUrl { get; set; } = "http://localhost/patch";

        public virtual string GetRemoteBuildsIndexUrl() =>
            PathsManager.UriCombine(RemoteUrl, BuildsFolderName, BuildsIndexFileName);

        public virtual string GetRemoteBuildDefinitionUrl(IVersion version) => PathsManager.UriCombine(RemoteUrl,
            BuildsFolderName, string.Format(BuildDefinitionFileName, version));

        public virtual string GetRemoteBuildUrl(IVersion version) =>
            PathsManager.UriCombine(RemoteUrl, BuildsFolderName, version.ToString());

        public virtual string GetPartialRemoteBuildUrl(IVersion version) =>
            PathsManager.UriCombine(BuildsFolderName, version.ToString());

        public virtual string GetRemotePatchesIndexUrl() =>
            PathsManager.UriCombine(RemoteUrl, PatchesFolderName, PatchesIndexFileName);

        public virtual string GetRemotePatchDefinitionUrl(IVersion from, IVersion to) =>
            PathsManager.UriCombine(RemoteUrl, PatchesFolderName, string.Format(PatchDefinitionFileName, from, to));

        public virtual string GetRemotePatchArchiveUrl(IVersion from, IVersion to) =>
            PathsManager.UriCombine(RemoteUrl, PatchesFolderName, string.Format(PatchFileName, from, to));

        public virtual string GetRemoteUpdaterIndexUrl() =>
            PathsManager.UriCombine(RemoteUrl, UpdaterFolderName, UpdaterIndexFileName);

        public virtual string GetRemoteUpdaterSafeModeIndexUrl() =>
            PathsManager.UriCombine(RemoteUrl, UpdaterFolderName, UpdaterSafeModeIndexFileName);

        public virtual string GetRemoteUpdaterSafeModeArchiveUrl(string archiveName) =>
            PathsManager.UriCombine(RemoteUrl, UpdaterFolderName, archiveName);

        public virtual string GetRemoteUpdaterFileUrl(string relativePath) =>
            PathsManager.UriCombine(RemoteUrl, UpdaterFolderName, relativePath);

        public bool PatcherUpdaterSafeMode { get; set; } = false;

        public bool EnableDiskSpaceCheck { get; set; } = true;

        public bool EnableRepairToTheLatestVersionIfNoPatchFound { get; set; } = true;
        ...
        public override string ToDebugString()
        {
            ...
            result += $"Remote URL => {RemoteUrl}\n" +
                      $"GetVersionFilePath() => {GetVersionFilePath()}\n" +
                      ...
                      $"EnableDiskSpaceCheck => {EnableDiskSpaceCheck}\n" +
                      $"EnableRepairToTheLatestVersionIfNoPatchFound => {EnableRepairToTheLatestVersionIfNoPatchFound}\n";
