...
namespace MHLab.Patch.Core.Client.IO
{
    public class FileDownloader : IDownloader
    {
        ...
        public virtual void Download(string url, string destFolder, Action<long> onChunkDownloaded)
        {
            ...
            if (!FileSystem.FileExists(downloadingTo))
            {
                ...
                using (var fs = FileSystem.GetFileStream(downloadingTo, FileMode.OpenOrCreate, FileAccess.Read,
                           FileShare.ReadWrite))
                {
                    fs.Flush();
                    fs.Dispose();
            ...
            url = DownloaderHelper.UrlEncode(url);

            var gotCanceled = false;

            using (var fs = FileSystem.GetFileStream(downloadingTo, FileMode.Open, FileAccess.ReadWrite,
                       FileShare.ReadWrite))
            {
                ...
                while (currentRetries < MaxDownloadRetries)
                {
                    ...
                    try
                    {
                        ...
                        try
                        {
                            data = DownloadData.Create(url, fs, Credentials, Proxy, ConfigureHttpRequest);
        ...
        protected virtual void ConfigureHttpRequest(HttpWebRequest request)
        {
        }
        ...
        protected virtual void SaveToFile(byte[] buffer, int count, Stream file, string fileName)
        {
            try
            {
    ...
    class DownloadData
    {
        ...
        public static DownloadData Create(string url, Stream stream, ICredentials credentials, IWebProxy proxy,
            Action<HttpWebRequest> configureHttpRequest)
        {
        ...
        private long GetFileSize(string url, ICredentials credentials, IWebProxy proxy)
        {
        ...
        private HttpWebRequest GetRequest(string url, ICredentials credentials, IWebProxy proxy)
        {
            if (url == null)
