    ...
    public class InventorySystemManagerItemSaver : SaverBase
    {
        ...
        public override void DeserializeAndLoadSaveData(Serialization serializedSaveData)
        {
            ...
            for (int i = 0; i < m_CurrentSaveData.Items.Length; i++) {
                ...
                // The item definition reference will not match the item definition ID anymore after a build patch.
                // That's why the item must be initialized using the Item definition ID and not the item definition reference.
